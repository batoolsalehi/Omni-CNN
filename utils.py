...
def load_layer_config(args,model,task):
    ...
    if args.dataset == 'cifar' or args.dataset == 'mixture':
        ...
        # fixed_layer = ['module.fc1.bias','module.fc2.bias','module.fc3.bias','module.fc4.bias']
        fixed_layer = ['module.hidden1.bias','module.hidden3.bias']
    ...
    if args.dataset == 'cifar' or args.dataset == 'mixture':
        ...
        output_layer = ['module.out.weight','module.out.bias']
    # For pruned layer
    config_setting = list(map(float, args.config_setting.split(",")))   # what is config setting? The partirioning ratio of network 2,4,4
    ...
    sparse_setting = 1-sparse_setting*args.config_shrink      #config-shrink ratio of model capacity to use
    with torch.no_grad():
        for name, W in (model.named_parameters()):
            ...
            if args.dataset == 'cifar' or args.dataset == 'mixture':
                ...
                if 'weight' in name and name!='module.out.weight' and name not in fixed_layer:   # do not change the last layer
                    prune_ratios[name] = sparse_setting
                    pruned_layer.append(name)
    ...
    print('Pruned layer:',pruned_layer)     #Pruned layer: ['conv1.weight', 'conv2.weight', 'conv3.weight', 'conv4.weight', 'fc1.weight', 'fc2.weight']
    print('Fixed layer:',fixed_layer)       #Fixed layer: ['module.fc1.bias']
    print('Output layer:',output_layer)     #Output layer: ['module.fc2.weight', 'module.fc2.bias']
    return args



def model_loader(args,task):
    print('args.adaptive_mask,args.arch',args.adaptive_mask,args.arch)
    if args.adaptive_mask:   #for sharing part
        from models.masknet import CifarNet, MnistNet, FlashNet
    else:
        from models.cifarnet import CifarNet
        from models.mnistnet import MnistNet
        from models.flash_net import FlashNet
    ...
    elif args.arch == 'flashnet':
        if task ==0:
            model = FlashNet('gps' ,args.classes)
        if task ==1:
            model = FlashNet('img' ,args.classes)
        if task ==2:
            model = FlashNet('lidar' ,args.classes)
...
def get_model_mask(model=None):
    masks = {}
    for name, W in (model.named_parameters()):
        print('model.named_parameters() in get_model_mask func',model.named_parameters())
        if 'mask' in name:
            continue
...
def set_adaptive_mask(model, reset=False, assign_value='', requires_grad=False):
    print('model.named_parameters() in set_adaptive_mask func',model.named_parameters())
    for name, W in (model.named_parameters()):
        if 'mask' in name:
            # set mask to be one
            if reset:
