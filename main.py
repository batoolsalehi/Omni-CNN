...
parser.add_argument('--rho-num', type=int, default = 3,
                    help ="define how many rohs for ADMM training")
parser.add_argument('--multi_rho', type=bool, default = True,
...
####
parser.add_argument('--sensetivity', default=0.02, type=float, help='sensetivity for stopping and pruning ratio')
parser.add_argument('--dynamic', default=False, type=str2bool, help='dynamic pruning ration')
...
def train(args, pipeline, task, train_loader,test_loader):   #1
    ...
    if args.adaptive_mask:
        set_adaptive_mask(model, reset=True, requires_grad=False)  #set weights to 1 if 'mask' in nam or assigned value and sets gradinets [fixing weighst]

    optimizer = torch.optim.Adam(model.parameters(), optimizer_init_lr,weight_decay=0.001)  #1
    ...
    for epoch in range(0, args.epochs):
        ...
        pipeline.train_model(args, model, args.mask, train_loader, criterion, optimizer, scheduler, epoch)  # for task0 args.mask is none

        end_train = time.time()
        print('***********test accuracy after each epoch*******')
        prec1 = pipeline.test_model(args, model,test_loader)
        end_test = time.time()
        print("Training time: {:.3f}; Testing time: {:.3f}.".format(end_train-start, end_test-end_train))
        print("#############Check if weighst are zeror during training###########",len(np.nonzero(model.conv1.weight.reshape(32*45*3*3).cpu().detach().numpy())[0]))
...
def prune(args, task, train_loader,test_loader):  #2
    print('*************** prune Model ***************')
    ...
    if args.admm_mask and task == args.tasks-1:
        args.admm = False
    admm_prune(args, args.mask, task, train_loader,test_loader)
    ...
    return masked_retrain(args, args.mask, task, train_loader,test_loader)


def admm_prune(args, pre_mask, task, train_loader,test_loader):  #3
    ...
    for epoch in range(1, args.epochs_prune + 1):
        ...
        if task!=0:
            ...
            for k in pre_mask_current_it.keys():
                print('(A==B).all()',k,(pre_mask_previous_it[k]==pre_mask_current_it[k]).all())
        prec1 = pipeline.test_model(args, model,test_loader)
    ...
    torch.save(model.state_dict(), save_path+"/prunned_{}{}_{}_{}_{}.pt".format(
        args.arch, args.depth, args.config_file, args.optmzr, args.sparsity_type))



def prune_train(args, pre_mask, ADMM, train_loader, criterion, optimizer, scheduler, epoch):  #4
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.mixup:
            ...
            ce_loss = criterion(output, target[:,0], smooth=args.smooth)
        mixed_loss = ce_loss
...
def masked_retrain(args, pre_mask, task, train_loader,test_loader):  #5
    ...
    '''
    save_path = os.path.join(args.save_path_exp,'task'+str(task))
    print("Loading file: "+save_path+"/prunned_{}{}_{}_{}_{}.pt".format(    ##upadted removed rho
        args.arch, args.depth, args.config_file, args.optmzr,
        args.sparsity_type))
    model.load_state_dict(torch.load(save_path+"/prunned_{}{}_{}_{}_{}.pt".format(
        args.arch, args.depth, args.config_file, args.optmzr,
        args.sparsity_type)))
...
if __name__ == '__main__':
    ...
    for task in range(num_tasks):
        ...
        '''
        model = model_loader(args,task)  #if args.adaptive_mask import from masknet otherwise models/flash_net
        model.cuda()
        print(model)
        args = load_layer_config(args, model, task)     #returns pruning ratio for each partrio based on config_setting [2,4,4] [1-2/10, 1-4/10,1-4/10]
        ...
        if args.dataset == 'cifar':
            ...
            train_loader, test_loader = pipeline.load_data_flash(args)
        """
        Pure train
        """
        ...
        if task == 0 and args.load_model_pruned:
            ...
            if task == 0 and args.load_model:
                ...
                train(args, pipeline ,task, train_loader,test_loader)
                # print('check',args.arch, args.depth)
        ...
        if task != num_tasks - 1:
            ...
            if task == 0 and args.load_model_pruned:
                ...
                mask_for_current_task = prune(args, task, train_loader,test_loader)
            ...
            test_sparsity_mask(args,mask_for_current_task)    #returns zero/non-zero statistics after pruning

            # print('mask_for_current_task, args.mask',mask_for_current_task, args.mask)
            cumulative_mask = mask_joint(args, mask_for_current_task, args.mask)
            args.mask = copy.deepcopy(cumulative_mask)       #be precise here about args.mask
        ...
        for i in range(task+1):
            ...
            # Trigger for experiment [leave space for future learning]\
            print('i and num_tasks',i,num_tasks)
            if i != num_tasks - 1:
                state_dict = torch.load(save_path + "/retrained.pt")
                load_state_dict(args, model, state_dict, target_keys=args.output_layer)
                prec1 = pipeline.test_model(args,model,test_loader,trained_mask)

            else: # Trigger for experiment [leave space for future learning]
                print('else for testing')
                if args.heritage_weight or args.adaptive_mask:
                    prec1 = pipeline.test_model(args,model,test_loader)
                elif args.adaptive_mask:
                    prec1 = pipeline.test_model(args,model,test_loader,trained_mask)
                else:
                    prec1 = pipeline.test_model(args,model,test_loader, mask_reverse(args, args.mask))


        args.load_model = save_path+"/cumu_model.pt"
        model = model_loader(args,task)
        model.cuda()
        if args.fixed_layer:
